package hu.valdar.progtech.gy9.sudoku.model;
...
public class SudokuLogic {
     ...
     */
    private boolean isPossibleBlock(int x, int y, int number) {
        // Meghatározzuk a kapott paraméterek alapján az aktuális blokk bal felső elemének indexét.
        int blockX = x < 3 ? 0 : x < 6 ? 3 : 6;
        int blockY = y < 3 ? 0 : y < 6 ? 3 : 6;
    ...
    private int getNextPossibleNumber(int x, int y) {
        ...
        while (numbers.size() > 0) {
            ...
            if (isPossibleX(y, number) && isPossibleY(x, number) && isPossibleBlock(x, y, number))
                return number;
        }
        return 0;
    }
