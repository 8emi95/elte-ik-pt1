...
import hu.elte.progtech.sudoku.model.field.ValueInitializer;
...
public class SudokuEngine {
	...
	private void initializeValues() {
		...
		getValueInitializer().initializeValues();
	}

	private ValueInitializer getValueInitializer() {
		if (size == 4) {
			return new ValueInitializer4x4(values);
		} else if (size == 9) {
			return new ValueInitializer9x9(values);
		} else if (size == 16) {
			return new ValueInitializer16x16(values);
		}
		return null;
	}
	...
	private boolean isCompleted() {
		// Note: cannot be "lazy" anymore,
		// since we need to update each and every "red" fields
		boolean completed = true;
		for (int i = 0; i < size; ++i) {
			for (int j = 0; j < size; ++j) {
				completed &= isValid(i, j);
			}
		}
		return completed;
	}
	...
	private boolean isValid(int row, int column) {
		if (values[row][column].getValue() != 0) {
			if (isUnique(row, column)) {
				return true;
			} else {
				view.invalidField(row, column);
			}
		}
		return false;
	}
