...
public class SudokuFrame extends JFrame implements SudokuView {
	...
	private void addField(int row, int column) {
		int value = engine.getValue(row, column);
		JButton field = new JButton(getLabelOfField(value));
		field.setBackground(getFieldColor(value));
	...
	public void updateFields() {
		for (int i = 0; i < engine.getSize(); ++i) {
			for (int j = 0; j < engine.getSize(); ++j) {
				int value = engine.getValue(i, j);
				JButton field = getField(i, j);
				field.setBackground(getFieldColor(value));
				field.setText(getLabelOfField(value));
			}
		}
	}

	private JButton getField(int row, int column) {
	...
	@Override
	public void invalidField(int row, int column) {
		getField(row, column).setBackground(RED);
	}
